...
void initParameters(double omega_0);
void initData();
void createArtificialDataset(double dt_data);
void createArrays();
void getNumberOfLines(const char* filename);
void setPadding();
int copyDataFromFileToArray(const char* filename, double *targetArray);
...
void WaveletAnalysisTD();
void WaveletAnalysisFD();
void freeData(Data_s* data);
...
int main(int argc, char* argv[]){
    initParameters(atof(argv[1]));
    initData();

    // Create artificial data (here a linear chirp function / a sin with constant frequency)
    createArtificialDataset(data.dt_data);

    getNumberOfLines("data_input/hellothere.dat");
    ...
    copyDataFromFileToArray("data_input/hellothere.dat", data.dataArray);
...
void initParameters(double omega_0){
    	parameters.freq_min = 0.1;
	parameters.freq_max = 20.0;	
	parameters.delta_freq = 0.01;
	parameters.time_min = 0.0;
	parameters.time_max = 20.0;
	parameters.delta_time = 0.01;
	parameters.b = 0.0;
	parameters.k = 0.0;
	parameters.omega_0 = omega_0;
}
...
void initData(){
    ...
    data.numLines = -1;
    data.padding = -1;
    data.numLines_padded = -1;
    data.dataArray = NULL;
    data.waveletArray = NULL;
    data.productArray = NULL;
}
...
void setPadding(){
    int padding = ceil(data.numLines / 4.0);
    printf("padding: %d\n", padding);
    data.padding = padding;
    data.numLines_padded = data.numLines + 2*padding;
